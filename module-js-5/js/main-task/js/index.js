...
const getUsersByEyeColor = (arr, color) => {
    return users.filter(arr => arr.eyeColor === color);
};
console.log(getUsersByEyeColor(users, 'blue'));

const getUsersByGender = (arr, gender) => {
    return users.filter(arr => arr.gender === gender).map(arr => arr.name);
};
console.log(getUsersByGender(users, 'male'));

const getInactiveUsers = arr => {
    return users.filter(arr => arr.isActive === false);
};

console.log(getInactiveUsers(users));

const getUserByEmail = (arr, email) => {
    return users.find(arr => arr.email === email);
};
...
const getUsersWithAge = (arr, min, max) => {
    return users.filter(arr => arr.age >= min && arr.age <= max)
};
console.log(getUsersWithAge(users, 20, 30));                                            
console.log(getUsersWithAge(users, 30, 40)); 

const getTotalBalance = arr => {
    return users.map(arr => arr.balance).reduce((acc,arr) => acc += arr,0);
};
console.log(getTotalBalance(users));

const getUsersByFriend = (arr, name) => {
    return users.filter(arr => arr.friends.includes(name)).map(arr => arr.name);
};
