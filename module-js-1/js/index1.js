...
seatsError = 'Желаемого вами количества мест нет в наличии';
...
if (Number.parseInt(destination) === 1) {
  ...
  if (Number.isNaN(seats) === true) {
    ...
    const seats = prompt("Вы летите в Хургаду. Введите количество необходимых мест");
    if (Number.isNaN(seats) === true) {
      alert(error);
    } else if (seats === null) {
      alert(weAreSorry);
...
} else if (Number.parseInt(destination) < 1 || Number.parseInt(destination) > 3) {
    alert('Выбранного вами направления нет в наличии! Вернитесь к списку!');
}
